...
// WeekdayQueueMonitor is a wrapper around the DefaultQueueMonitor that disables queue monitoring on weekends and during off hours (05:00 - 18:00 UTC).
...
func (w *WeekdayQueueMonitor) CheckAndProcessStatus(ctx context.Context) error {
	if w.isDuwOffTime() {
		w.log.Debug("Queue monitoring is disabled on weekends and off hours (05:00 - 18:00 UTC), skipping status check")
		return nil
	}
